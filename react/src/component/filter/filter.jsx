...
export default function Filter() {
  ...
  //   },[() => setTemp,])
    ...
    function setTemp(filter)
    {
      ...
      if(true)
      {
        ...
        temporaryActiveFilters.remove(filter);
      }
  ...
  <>
    ...
    <Pills types={mealType} activeFilter={activeFilters} />
    <hr />
    <SectionMobile header="Meal Allergies/Diet"/>
    <Pills types={allergies} activeFilter={activeFilters} />
    <hr />
    <SectionMobile header="Drinks"/>
    <Pills types={proteins} activeFilter={activeFilters} />
    <hr />
...
function SectionMobile({header}){
  return(
    <>
      ...
      <img className="arrow" src={arrowDown} alt="Arrow down" style={{maxWidth : '30px'}} />
      {/* </button> */}
      </div>
      <hr />
    </>
...
function ApplyClearPills({types, id}){
  ...
    //     <DesktopCheckbox types={proteins}/>
